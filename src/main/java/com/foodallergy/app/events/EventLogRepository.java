...
public interface EventLogRepository extends CrudRepository<EventLog, String> {
    ...
    @Query(value = "select event_id from event_log where date_occured in (:datesToFilterBy) group by event_id order by count(*) desc", nativeQuery = true)
    List<Object> getEventsByDates(@Param("datesToFilterBy") ArrayList<LocalDate> datesToFilterBy);
}